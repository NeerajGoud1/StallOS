  ...
  <div className="h-full bg-white rounded-2xl shadow-sm border border-gray-200 p-8 overflow-y-auto">
    ...
    <div className="bg-green-50 p-6 rounded-lg shadow-inner mb-6">
      <h2 className="text-xl font-bold text-green-600 mb-4">
        Daily Ingredient Prices (₹ per kg/L)
      </h2>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {Object.keys(ingredientPrices).map((ing) => (
          <div key={ing} className="flex flex-col">
            <label
              htmlFor={ing}
              className="capitalize text-sm font-medium text-gray-700 mb-1"
            >
              {ing}:
            </label>
            <input
        ...
        {menuItems.map((item: any) => (
          <div key={item.id} className="flex flex-col">
            ...
            <div className="flex items-center space-x-2">
              <button
                onClick={() => handlePlatesSoldChange(item.id, -1)}
                className="p-1.5 bg-red-400 text-white rounded-full shadow-md hover:bg-red-500 transition-colors focus:outline-none focus:ring-2 focus:ring-red-300"
              >
                <svg
              ...
              <button
                onClick={() => handlePlatesSoldChange(item.id, 1)}
                className="p-1.5 bg-green-400 text-white rounded-full shadow-md hover:bg-green-500 transition-colors focus:outline-none focus:ring-2 focus:ring-green-300"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-5 w-5"
    ...
    <div className="bg-purple-50 p-6 rounded-lg shadow-inner border-l-4 border-purple-400 mb-6">
      <h2 className="text-xl font-bold text-purple-700 mb-2">
        StallOS AI Recommendation
      </h2>
      <p
        className="text-md text-gray-800 leading-relaxed"
        dangerouslySetInnerHTML={{
          __html: aiRecommendation.replace(
            /\*\*(.*?)\*\*/g,
            "<strong>$1</strong>"
          ),
        }}
    ...
    <div className="bg-gray-50 p-6 rounded-lg shadow-inner">
      ...
      <div className="flex flex-col sm:flex-row justify-end items-end sm:items-center space-y-2 sm:space-y-0 sm:space-x-6 text-xl font-extrabold text-gray-800">
        <span className="bg-gray-200 p-2 px-4 rounded-lg shadow-sm">
          Total Revenue: ₹{dailyPerformance.totalRevenue.toFixed(2)}
        </span>
        <span className="bg-green-200 p-2 px-4 rounded-lg shadow-sm text-green-900">
          Total Profit: ₹{dailyPerformance.totalOverallProfit.toFixed(2)}
        </span>
      </div>
    </div>
  </div>
...
const PerformanceAnalytics = ({
  ...
  menuData: any[];
  dailyPerformance: { totalRevenue: number; totalOverallProfit: number };
}) => {
      ...
      datasets: [
        {
          ...
          backgroundColor: positiveProfitItems.map(
  ...
  const profitDistributionOptions: any = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Profit Contribution by Item (Today)",
      ...
      tooltip: {
        callbacks: {
          label: function (context: any) {
            ...
            return `${context.label}: ₹${Number(context.raw).toFixed(
              2
            )} (${percentage}%)`;
          },
        },
  ...
  const salesVsMarginData = useMemo(() => {
    const sortedData = [...menuData].sort(
      (a, b) => b.platesSold - a.platesSold
    );
    return {
      ...
      datasets: [
        {
          label: "Plates Sold",
          data: sortedData.map((item) => item.platesSold),
  ...
  const salesVsMarginOptions: any = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Sales Volume vs. Profit Margin",
        font: { size: 18, weight: "bold" },
      },
    },
    scales: {
      "y-sales": {
        type: "linear",
        position: "left",
        title: { display: true, text: "Plates Sold" },
      },
      "y-margin": {
        type: "linear",
        position: "right",
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-blue-50 p-6 rounded-lg shadow-lg text-center border border-blue-200">
          ...
          <p className="text-4xl font-bold text-blue-700">
            ₹{dailyPerformance.totalRevenue.toFixed(2)}
          </p>
        </div>
        <div className="bg-green-50 p-6 rounded-lg shadow-lg text-center border border-green-200">
          ...
          <p className="text-4xl font-bold text-green-700">
            ₹{dailyPerformance.totalOverallProfit.toFixed(2)}
          </p>
        </div>
      </div>
...
const LandingPage = () => (
  // CHANGE 2: The classes here have been modified to fix the alignment.
  // 'justify-center' was changed to 'justify-start' to align content to the top.
  <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 h-full flex flex-col justify-start items-center">
    {/* A 'pt-16' class (padding-top) was added here to give some space from the top edge. */}
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-center">
            <button
              ...
              className="px-6 py-2 bg-green-600 text-white rounded-xl font-semibold hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition duration-150 ease-in-out"
            >
        ...
        <div className="mt-10">
          ...
          <div className="space-y-4 text-gray-700">
            <ul className="list-disc list-inside space-y-2">
              <li>
                <span className="font-semibold">Review Daily:</span> Use this
                tool daily to adapt to changing ingredient prices and sales
                trends.
              </li>
              <li>
                <span className="font-semibold">Experiment with Pricing:</span>{" "}
                Based on the AI's suggestions, try small price adjustments and
                monitor their impact on sales and profit.
              </li>
              <li>
                <span className="font-semibold">
                  Promote High-Profit Items:
                </span>{" "}
                If the strategy suggests focusing on certain items, promote them
                actively through daily specials or visible displays.
              </li>
              <li>
                <span className="font-semibold">Optimize Inventory:</span> Use
                insights on ingredient prices and sales to manage your stock
                efficiently, reducing waste.
              </li>
              <li>
                <span className="font-semibold">Gather Customer Feedback:</span>{" "}
                Combine AI insights with direct customer feedback to refine your
                menu and offerings.
              </li>
              <li>
                <span className="font-semibold">Track Performance:</span> Keep a
                record of your daily sales and profit after implementing
                strategies to measure effectiveness over time.
              </li>
            </ul>
          </div>
        </div>
...
function App() {
  ...
  const menuItems = useMemo(
    () => [
      ...
      },
      {
        ...
        name: "Plain Dosa",
        sellingPrice: 50,
        ingredients: { rice: 0.08, lentils: 0.02, oil: 0.005, salt: 0.001 },
      },
    ],
    []
  );
  const [platesSold, setPlatesSold] = useState(() => {
    ...
    menuItems.forEach((item) => {
      initialSales[item.id] = 10 + Math.floor(Math.random() * 50);
    });
    return initialSales;
  });
  ...
  const calculateMenuMetrics = useCallback(() => {
    ...
    const calculatedData = menuItems.map((item) => {
      const totalCost = Object.entries(item.ingredients).reduce(
        (acc, [ingredient, quantity]) =>
          acc +
          quantity *
            (ingredientPrices[ingredient as keyof typeof ingredientPrices] ||
              0),
        0
      );
      const profitPerPlate = item.sellingPrice - totalCost;
      const profitMargin =
        item.sellingPrice > 0 ? (profitPerPlate / item.sellingPrice) * 100 : 0;
  ...
  const generateStaticAIRecommendation = useCallback(() => {
    if (
      !menuData ||
      menuData.length === 0 ||
      menuData.every((item) => item.platesSold === 0)
    ) {
      setAiRecommendation(
        "Enter today's sales to get a profit recommendation."
      );
      return;
    }
    const sortedByProfit = [...menuData].sort(
      (a, b) => b.profitMargin - a.profitMargin
    );
    const highestProfitItem = sortedByProfit[0];
    const lowerProfitItemsWithSales = sortedByProfit.filter(
      (item) =>
        item.id !== highestProfitItem.id &&
        item.platesSold > 0 &&
        item.profitMargin < highestProfitItem.profitMargin - 5
    );
    if (
      highestProfitItem &&
      highestProfitItem.profitMargin > 20 &&
      lowerProfitItemsWithSales.length > 0
    ) {
      const lowerProfitItem = lowerProfitItemsWithSales[0];
      const profitGainPerPlateShift =
        highestProfitItem.profitPerPlate - lowerProfitItem.profitPerPlate;
      const potentialExtraProfit = profitGainPerPlateShift * 10;
      setAiRecommendation(
        `**${
          highestProfitItem.name
        }** has a **${highestProfitItem.profitMargin.toFixed(
          0
        )}% margin**. Suggesting it over **${
          lowerProfitItem.name
        }** could earn an extra **₹${potentialExtraProfit.toFixed(
          2
        )}** for every 10 plates shifted.`
      );
    } else if (highestProfitItem) {
      setAiRecommendation(
        `Market is stable. **${
          highestProfitItem.name
        }** is your top earner with a **${highestProfitItem.profitMargin.toFixed(
          0
        )}% profit margin**. Keep it up!`
      );
    }
  }, [menuData]);
  ...
  useEffect(() => {
    generateStaticAIRecommendation();
  }, [generateStaticAIRecommendation]);
  ...
  const features = [
    {
      id: "ai-chef",
      name: "AI Sous-Chef",
      description: "Recipe adjustments via AI",
      icon: Chef,
    ...
    },
    {
      ...
      description: "Edit prices & sales data",
      icon: TrendingUp,
    ...
    },
    {
      ...
      description: "View dynamic charts",
      icon: BarChart3,
      color: "text-pink-600",
      bgColor: "bg-pink-50",
    },
    {
      id: "zero-waste",
      name: "Zero-Waste Genius",
      description: "Convert leftovers into profit",
      icon: Recycle,
      color: "text-blue-600",
      bgColor: "bg-blue-50",
    },
    {
      id: "chaat-gpt",
      name: "Chaat-GPT Voice",
      description: "Voice-powered intelligence",
      icon: Mic,
      color: "text-purple-600",
    ...
    {
      id: "daily-strategy",
      name: "Daily Sales Strategy",
      description: "Market-based recommendations",
      icon: Target,
      ...
      case "profit-optimizer":
        return (
          <ProfitOptimizer
            ...
            platesSold={platesSold}
            menuItems={menuItems}
            handlePriceChange={handlePriceChange}
            handlePlatesSoldChange={handlePlatesSoldChange}
  ...
  return (
    // CHANGE 1: These classes on the main div create the sticky sidebar layout.
    // 'h-screen' and 'overflow-hidden' are the key changes.
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl transition-all duration-200 group relative ${
                    isActive
                      ? `${feature.bgColor} border-opacity-20 border-current`
                      : "hover:bg-gray-50"
                  }`}
                >
                      ...
                      <div className="flex items-center justify-between">
                        <p
                          className={`text-sm font-medium ${
                            isActive ? feature.color : "text-gray-900"
                          }`}
                        >
                          {feature.name}
                        </p>
            ...
            <div className="space-y-1">
              {quickActions.map((action, index) => {
                ...
                return (
                  <button
                    key={index}
                    className="w-full text-left p-3 rounded-lg hover:bg-gray-50 transition-colors group flex items-center space-x-3"
                  >
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs font-semibold text-green-600">
                  {(
                    (dailyPerformance.totalOverallProfit /
                      dailyPerformance.totalRevenue) *
                      100 || 0
                  ).toFixed(1)}
                  %
                </span>
